...
def main(argv):
    ...
    parser.add_argument("--marker_width", help="Length from center of top-left circle to center of top-right circle (cm)", default=15.0, type=float)
    parser.add_argument("--marker_height", help="Length from center of top-left circle to center of bottom-left circle (cm)", default=10.0, type=float)
    ...
    d_line0_AB = args.AB0 #AB
    d_line0_BD = 1 - args.AB0 #BD
    d_line0_AC = args.AC0 #//AC
    d_line0_CD = 1 - args.AC0 #//CD
    cross_ration_0 = (d_line0_AB/d_line0_BD)/(d_line0_AC/d_line0_CD)
    
    d_line1_AB = args.AB1 #AB
    d_line1_BD = 1 - args.AB1 #BD
    d_line1_AC = args.AC1 #;//AC
    d_line1_CD = 1 - args.AC1 #;//CD
    ...
    cx = circle_clearance + circle_radius + marker_width / 2
    cy = circle_clearance + circle_radius + marker_height / 2
    
    
    
    #Squares for marker size reference
    size_inner_rect_width = marker_width - circle_radius * 2 - circle_clearance * 2
    size_inner_rect_height = marker_height - circle_radius * 2 - circle_clearance * 2
    corner_inner_square = 2*circle_radius + 2*circle_clearance
    size_outer_rect_width = marker_width + circle_radius * 2 + circle_clearance * 2
    size_outer_rect_height = marker_height + circle_radius * 2 + circle_clearance * 2
    ...
    #Outer square
    marker.add(svgwrite.shapes.Rect((0*cm,0*cm),(size_outer_rect_width*cm,size_outer_rect_height*cm), fill='white', stroke='black'))
    
    #inner square
    marker.add(svgwrite.shapes.Rect((corner_inner_square*cm,corner_inner_square*cm),(size_inner_rect_width*cm,size_inner_rect_height*cm), fill='none', stroke='black'))
    
    
    
    
    x1 = circle_radius + circle_clearance
    y1 = circle_radius + circle_clearance
    x2 = circle_radius + circle_clearance + marker_width
    y2 = circle_radius + circle_clearance + marker_height
    ...
    marker.add(svgwrite.shapes.Circle(((x1+marker_width*CR_Line0_AB)*cm,y1*cm), circle_radius*cm))
    marker.add(svgwrite.shapes.Circle(((x1+marker_width*CR_Line0_AC)*cm,y1*cm), circle_radius*cm))

    marker.add(svgwrite.shapes.Circle((x1*cm,(y1+marker_height*CR_Line0_AB)*cm), circle_radius*cm))
    marker.add(svgwrite.shapes.Circle((x1*cm,(y1+marker_height*CR_Line0_AC)*cm), circle_radius*cm))
    
    #Now we draw the Cross related circles for line 1
    marker.add(svgwrite.shapes.Circle(((x1+marker_width*CR_Line1_AB)*cm,y2*cm), circle_radius*cm))
    marker.add(svgwrite.shapes.Circle(((x1+marker_width*CR_Line1_AC)*cm,y2*cm), circle_radius*cm))

    marker.add(svgwrite.shapes.Circle((x2*cm,(y1+marker_height*CR_Line1_AB)*cm), circle_radius*cm))
    marker.add(svgwrite.shapes.Circle((x2*cm,(y1+marker_height*CR_Line1_AC)*cm), circle_radius*cm))
    
    
    #Put text with information about the marker in the center
    t_x = cx - 5
    t_y = cy + size_outer_rect_height/2 +1
    ...
    if args.A4:
        ...
        x_m = ((w/2)-(size_outer_rect_width/2))
        y_m = ((h/2)-(size_outer_rect_height/2))
